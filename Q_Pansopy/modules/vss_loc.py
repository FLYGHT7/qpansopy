...
def calculate_vss_loc(iface, point_layer, runway_layer, params):
    ...
    # Calculate VSS parameters for ILS/LOC/APV
    # For ILS/LOC/APV, the VSS is slightly different from Straight In NPA
    vss_length = 1.5 * (och - rdh) / tan(radians(vpa))
    ...
    # Create VSS layer
    vss_layer = QgsVectorLayer(f"PolygonZ?crs={map_srid}", "VSS_ILS_LOC_APV", "memory")
    ...
    # Create OCS layer
    ocs_layer = QgsVectorLayer(f"PolygonZ?crs={map_srid}", "OCS_ILS_LOC_APV", "memory")
    ...
    if export_kml and output_dir:
        # Define file paths
        vss_path = os.path.join(output_dir, 'vss_ils_loc_apv_layer.kml')
        ocs_path = os.path.join(output_dir, 'ocs_ils_loc_apv_layer.kml')
